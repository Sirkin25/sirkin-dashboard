...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      recommendations: getRecommendations(envCheck, connectionTest),
    })
  } catch (error) {
    return NextResponse.json(
      {
...
function getRecommendations(envCheck: any, connectionTest: any): string[] {
  const recommendations: string[] = []

  if (!envCheck.hasPrivateKey) {
    recommendations.push("Missing GOOGLE_PRIVATE_KEY environment variable")
  } else if (!envCheck.privateKeyStartsCorrectly || !envCheck.privateKeyEndsCorrectly) {
    recommendations.push(
      "GOOGLE_PRIVATE_KEY format is incorrect - should start with '-----BEGIN PRIVATE KEY-----' and end with '-----END PRIVATE KEY-----'",
    )
  }

  if (!envCheck.hasClientEmail) {
    recommendations.push("Missing GOOGLE_SERVICE_ACCOUNT_EMAIL environment variable")
  }
